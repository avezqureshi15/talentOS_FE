...
const ApplicantDetailsModal = ({ open, applicantName, details, onClose, isRemote = false }: ApplicantDetailsModalProps) => {
  const hasCandidateDetails = Boolean(
    ...
    details.howDidYouHear ||
    details.willingToRelocate,
  );
        ...
        <div className="ad-grid">
          {isRemote && (
            <div className="ad-row">
              <span className="ad-label">{APPLICANT_LABELS.ROLE_TYPE}</span>
              <span className="ad-value ad-value--remote">{APPLICANT_LABELS.REMOTE_ROLE}</span>
            </div>
          )}
