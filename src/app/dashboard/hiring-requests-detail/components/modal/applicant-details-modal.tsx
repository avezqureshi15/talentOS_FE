...
const ApplicantDetailsModal = ({ open, applicantName, details, onClose }: ApplicantDetailsModalProps) => (
        ...
        {(Object.keys(LABEL_MAP) as (keyof typeof LABEL_MAP)[]).map((key) => {
          ...
          return (
            <div className="ad-row" key={key}>
              ...
              <span className="ad-value">{value === "immediate" ? APPLICANT_DETAILS_MODAL.IMMEDIATE : value}{suffix}</span>
            </div>
          );
        })}
        {(!details.currentCtc && !details.expectedCtc && !details.location && !details.yearsOfExperience && !details.noticePeriod && !details.howDidYouHear) && (
